...
if isa(fun, 'function_handle') && isequal(fun, @mean)
    val = squeeze(permute(accumarray(Xind, I, sizeXg,@(x)mean_omitnan(x)),N:-1:1));
else
...
if nDims ==1
 ...
 function result = mean_omitnan(in)
        result = mean(in,'omitnan');
 end
